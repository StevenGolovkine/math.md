...
The symmetric difference is important in {{< backlink "1778157439-measure-theory" "measure theory" >}}, because we say that two sets are the same up to measure zero. This idea can be expressed using the symmetric difference. Thus, $\mu(A \bigtriangleup B) = 0$ means that $A$ and $B$ may not be exactly equal, but they differ only on a negligible set.
