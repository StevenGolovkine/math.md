...
{{< admonition type="tip" title="Properties" >}}
* $f_{1} = O(g_{1})$ and $f_{2} = O(g_{2}) \implies f_{1} f_{2} = O(g_{1} g_{2})$.
* $f \cdot O(g) = O(fg)$.
* $f_{1} = O(g_{1})$ and $f_{2} = O(g_{2}) \implies f_{1} + f_{2} = O(\max(|g_{1}|, |g_{2}|))$.
* If $f = O(g)$, then, for $k > 0$, $k \cdot f = O(g)$.
{{< /admonition >}}
